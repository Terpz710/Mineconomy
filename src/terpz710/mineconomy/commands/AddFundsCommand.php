...
class AddFundsCommand extends BaseCommand {
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void{
        ...
        if ($plugin->hasBalance($targetName)) {
            $plugin->addFunds($targetName, $amount);
            $sender->sendMessage((string) new Message("successfully-added-funds", ["{name}", "{amount}"], [$targetName, number_format($amount)]));
        } else {
